...
class Application {
    ...
    postCountry() {
        fetch("http://127.0.0.1:3001/api/country", {
    ...
    postCity() {
        fetch("http://127.0.0.1:3001/api/city", {
            body: JSON.stringify({"postCity":222}),
            headers: {
                "Content-Type": "application/json" // important for bodyParser(!)
            },
            method: "POST"
            })
            .then(response => response.json())
    ...
    bindAllItemsHandler() {
        document.getElementById('all').addEventListener('click', () => {
            ...
            for(const key in this.countries.data) {
                this.countries.data[key].selected = false;
            }
